...
ffi.cdef [[
  ...
  BOOL FaxSetConfiguration(          HANDLE FaxHandle, const FAX_CONFIGURATION* FaxConfig);
  BOOL FaxSetGlobalRoutingInfo(      HANDLE FaxHandle, const FAX_GLOBAL_ROUTING_INFO* RoutingInfo);
  BOOL FaxSetLoggingCategories(      HANDLE FaxHandle, const FAX_LOG_CATEGORY* Categories, DWORD NumberCategories);
  ...
  BOOL FaxSetPort(                   HANDLE FaxPortHandle, const FAX_PORT_INFO* PortInfo);
  ...
  BOOL FaxPrintCoverPage(            const FAX_CONTEXT_INFO* FaxContextInfo, const FAX_COVERPAGE_INFO* CoverPageInfo);
  BOOL FaxStartPrintJob(             LPCTSTR PrinterName, const FAX_PRINT_INFO* PrintInfo, LPDWORD FaxJobId, PFAX_CONTEXT_INFO FaxContextInfo);
  ...
  BOOL FaxSetJob(                    HANDLE FaxHandle, DWORD JobId, DWORD Command, const FAX_JOB_ENTRY* JobEntry);
  ...
  BOOL FaxSendDocument(              HANDLE FaxHandle, LPCTSTR FileName, PFAX_JOB_PARAM JobParams, const FAX_COVERPAGE_INFO* CoverpageInfo, LPDWORD FaxJobId);
  BOOL FaxSetRoutingInfo(            HANDLE FaxPortHandle, LPCTSTR RoutingGuid, const BYTE* RoutingInfoBuffer, DWORD RoutingInfoBufferSize);
]]
