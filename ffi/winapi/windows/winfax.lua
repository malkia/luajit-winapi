...
ffi.cdef [[
  ...
  BOOL FaxSetConfiguration(          HANDLE FaxHandle, WINAPI_FAX_CONFIGURATION* FaxConfig);
  BOOL FaxSetGlobalRoutingInfo(      HANDLE FaxHandle, WINAPI_FAX_GLOBAL_ROUTING_INFO* RoutingInfo);
  BOOL FaxSetLoggingCategories(      HANDLE FaxHandle, WINAPI_FAX_LOG_CATEGORY* Categories, DWORD NumberCategories);
  ...
  BOOL FaxSetPort(                   HANDLE FaxPortHandle, WINAPI_FAX_PORT_INFO* PortInfo);
  ...
  BOOL FaxPrintCoverPage(            WINAPI_FAX_CONTEXT_INFO* FaxContextInfo, WINAPI_FAX_COVERPAGE_INFO* CoverPageInfo);
  BOOL FaxStartPrintJob(             LPCTSTR PrinterName, WINAPI_FAX_PRINT_INFO* PrintInfo, LPDWORD FaxJobId, PFAX_CONTEXT_INFO FaxContextInfo);
  ...
  BOOL FaxSetJob(                    HANDLE FaxHandle, DWORD JobId, DWORD Command, WINAPI_FAX_JOB_ENTRY* JobEntry);
  ...
  BOOL FaxSendDocument(              HANDLE FaxHandle, LPCTSTR FileName, PFAX_JOB_PARAM JobParams, WINAPI_FAX_COVERPAGE_INFO* CoverpageInfo, LPDWORD FaxJobId);
  BOOL FaxSetRoutingInfo(            HANDLE FaxPortHandle, LPCTSTR RoutingGuid, WINAPI_BYTE* RoutingInfoBuffer, DWORD RoutingInfoBufferSize);
]]
return ffi.load( 'WinFax.dll' )
