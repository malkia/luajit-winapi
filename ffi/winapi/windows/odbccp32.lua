...
ffi.cdef [[
  BOOL    SQLConfigDataSource(          HWND hwndParent, ODBC_DSN_REQUEST fRequest, LPCTSTR lpszDriver, LPCTSTR lpszAttributes);
  BOOL    SQLConfigDriver(              HWND hwndParent, ODBC_DRIVER_REQUEST fRequest, LPCTSTR lpszDriver, LPCTSTR lpszArgs, LPTSTR lpszMsg, WORD cbMsgMax, WORD* pcbMsgOut);
  BOOL    SQLCreateDataSource(          HWND hwnd, LPTSTR lpszDS);
  BOOL    SQLGetConfigMode(             ODBC_CONFIG_MODE* pwConfigMode);
  ...
  BOOL    SQLInstallDriverEx(           LPCTSTR lpszDriver, LPCTSTR lpszPathIn, LPTSTR lpszPathOut, WORD cbPathOutMax, WORD* pcbPathOut, ODBC_INSTALL_REQUEST fRequest, LPDWORD lpdwUsageCount);
  BOOL    SQLInstallDriverManager(      LPTSTR lpszPath, WORD cbPathMax, WORD* pcbPathOut);
  RETCODE SQLInstallerError(            WORD iError, ODBC_ERROR_CODE* pfErrorCode, LPTSTR lpszErrorMsg, WORD cbErrorMsgMax, WORD* pcbErrorMsg);
  ...
  RETCODE SQLPostInstallerError(        ODBC_ERROR_CODE fErrorCode, LPTSTR szErrorMsg);
  ...
  BOOL    SQLSetConfigMode(             ODBC_CONFIG_MODE wConfigMode);
