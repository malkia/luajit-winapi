...
//static const WINAPI_PANOSE_XHEIGHT PAN_NO_FIT = 1;
  ...
  HBITMAP                CreateDIBitmap(               HDC hdc, const BITMAPINFOHEADER* lpbmih, WINAPI_CreateBitmapInitFlag fdwInit, const VOID* lpbInit, const BITMAPINFO* lpbmi, WINAPI_ColorUseEnum fuUsage);
  HBITMAP                CreateDIBSection(             HDC hdc, const BITMAPINFO* pbmi, WINAPI_ColorUseEnum iUsage, VOID** ppvBits, HANDLE hSection, DWORD dwOffset);
  ...
  int                    SetDIBits(                    HDC hdc, HBITMAP hbmp, UINT uStartScan, UINT cScanLines, const VOID* lpvBits, const BITMAPINFO* lpbmi, WINAPI_ColorUseEnum fuColorUse);
  int                    SetDIBitsToDevice(            HDC hdc, int XDest, int YDest, DWORD dwWidth, DWORD dwHeight, int XSrc, int YSrc, UINT uStartScan, UINT cScanLines, const VOID* lpvBits, const BITMAPINFO* lpbmi, WINAPI_ColorUseEnum fuColorUse);
  ...
  int                    StretchDIBits(                HDC hdc, int XDest, int YDest, int nDestWidth, int nDestHeight, int XSrc, int YSrc, int nSrcWidth, int nSrcHeight, const VOID* lpBits, const BITMAPINFO* lpBitsInfo, WINAPI_ColorUseEnum iUsage, WINAPI_RasterOperationEnum dwRop);
  ...
  HBRUSH                 CreateDIBPatternBrushPt(      const VOID* lpPackedDIB, WINAPI_ColorUseEnum iUsage);
  ...
  BOOL                   ModifyWorldTransform(         HDC hdc, const XFORM* lpXform, WINAPI_ModifyWorldTransformMode iMode);
  ...
  BOOL                   ExtTextOut(                   HDC hdc, int X, int Y, WINAPI_ExtTextOutFlags fuOptions, const RECT* lprc, LPCTSTR lpString, UINT cbCount, const INT* lpDx);
  ...
  DWORD                  GetGlyphOutline(              HDC hdc, UINT uChar, WINAPI_GetGlyphOutlineFormat uFormat, LPGLYPHMETRICS lpgm, DWORD cbBuffer, LPVOID lpvBuffer, const MAT2* lpmat2);
  ...
  UINT                   SetBoundsRect(                HDC hdc, const RECT* lprcBounds, WINAPI_BoundsAccumulationFlags flags);
  ...
  HRGN                   CreatePolygonRgn(             const POINT* lppt, int cPoints, WINAPI_PolyFillModes fnPolyFillMode);
  HRGN                   CreatePolyPolygonRgn(         const POINT* lppt, const INT* lpPolyCounts, int nCount, WINAPI_PolyFillModes fnPolyFillMode);
