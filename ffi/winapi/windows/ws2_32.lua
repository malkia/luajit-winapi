...
ffi.cdef [[
  ...
  WINAPI_SocketCode_int   connect(                         SOCKET s, WINAPI_struct sockaddr* name, int namelen);
  void                    freeaddrinfo(                    struct addrinfo* ai);
  WINAPI_WSA_ERROR        getaddrinfo(                     WINAPI_char* nodename, WINAPI_char* servname, WINAPI_struct addrinfo* hints, struct addrinfo** res);
  void                    FreeAddrInfoEx(                  PADDRINFOEX pAddrInfo);
  WINAPI_WSA_ERROR        GetAddrInfoEx(                   PCTSTR pName, PCTSTR pServiceName, WINAPI_NameSpaceFlags dwNameSpace, LPGUID lpNspId, WINAPI_ADDRINFOEX* pHints, PADDRINFOEX* ppResult, struct timeval* timeout, LPOVERLAPPED lpOverlapped, LPLOOKUPSERVICE_COMPLETION_ROUTINE lpCompletionRoutine, LPHANDLE lpNameHandle);
  void                    FreeAddrInfoW(                   PADDRINFOW pAddrInfo);
  WINAPI_WSA_ERROR        GetAddrInfoW(                    PCWSTR pNodeName, PCWSTR pServiceName, WINAPI_ADDRINFOW* pHints, PADDRINFOW* ppResult);
  struct hostent*         gethostbyaddr(                   WINAPI_char* addr, int len, WINAPI_AddressFamily type);
  struct hostent*         gethostbyname(                   WINAPI_char* name);
  WINAPI_SocketCode_int   gethostname(                     char* name, int namelen);
  WINAPI_WSA_ERROR        getnameinfo(                     WINAPI_struct sockaddr* sa, socklen_t salen, char* host, DWORD hostlen, char* serv, DWORD servlen, WINAPI_NI_Flags flags);
  WINAPI_WSA_ERROR        GetNameInfoW(                    WINAPI_SOCKADDR* pSockaddr, socklen_t SockaddrLength, PWCHAR pNodeBuffer, DWORD NodeBufferSize, PWCHAR pServiceBuffer, DWORD ServiceBufferSize, WINAPI_NI_Flags Flags);
  WINAPI_SocketCode_int   getpeername(                     SOCKET s, struct sockaddr* name, int* namelen);
  struct PROTOENT*        getprotobyname(                  WINAPI_char* name);
  struct PROTOENT*        getprotobynumber(                int number);
  struct servent*         getservbyname(                   WINAPI_char* name, WINAPI_char* proto);
  struct servent*         getservbyport(                   int port, WINAPI_char* proto);
  ...
  WINAPI_INET_ADDR_OUTPUT inet_addr(                       WINAPI_char* cp);
  ...
  WINAPI_SocketCode_int   select(                          int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, WINAPI_struct timeval* timeout);
  WINAPI_SocketCode_int   send(                            SOCKET s, LPVOID buf, int len, WINAPI_SendRecvFlags flags);
  WINAPI_SocketCode_int   sendto(                          SOCKET s, LPVOID buf, int len, WINAPI_SendRecvFlags flags, WINAPI_struct sockaddr* to, int tolen);
  ...
  HANDLE                  WSAAsyncGetHostByAddr(           HWND hWnd, unsigned int wMsg, WINAPI_char* addr, int len, int type, char* buf, int buflen);
  HANDLE                  WSAAsyncGetHostByName(           HWND hWnd, unsigned int wMsg, WINAPI_char* name, char* buf, int buflen);
  HANDLE                  WSAAsyncGetProtoByName(          HWND hWnd, unsigned int wMsg, WINAPI_char* name, char* buf, int buflen);
  HANDLE                  WSAAsyncGetProtoByNumber(        HWND hWnd, unsigned int wMsg, int number, char* buf, int buflen);
  HANDLE                  WSAAsyncGetServByName(           HWND hWnd, unsigned int wMsg, WINAPI_char* name, WINAPI_char* proto, char* buf, int buflen);
  HANDLE                  WSAAsyncGetServByPort(           HWND hWnd, unsigned int wMsg, int port, WINAPI_char* proto, char* buf, int buflen);
  ...
  WINAPI_SocketCode_int   WSAConnect(                      SOCKET s, WINAPI_struct sockaddr* name, int namelen, LPWSABUF lpCallerData, LPWSABUF lpCalleeData, LPQOS lpSQOS, LPQOS lpGQOS);
  BOOL                    WSAConnectByList(                SOCKET s, PSOCKET_ADDRESS_LIST SocketAddressList, LPDWORD LocalAddressLength, LPSOCKADDR LocalAddress, LPDWORD RemoteAddressLength, LPSOCKADDR RemoteAddress, WINAPI_struct timeval* timeout, LPWSAOVERLAPPED Reserved);
  BOOL                    WSAConnectByName(                SOCKET s, LPSTR nodename, LPSTR servicename, LPDWORD LocalAddressLength, LPSOCKADDR LocalAddress, LPDWORD RemoteAddressLength, LPSOCKADDR RemoteAddress, WINAPI_struct timeval* timeout, LPWSAOVERLAPPED Reserved);
  ...
  SOCKET                  WSAJoinLeaf(                     SOCKET s, WINAPI_struct sockaddr* name, int namelen, LPWSABUF lpCallerData, LPWSABUF lpCalleeData, LPQOS lpSQOS, LPQOS lpGQOS, DWORD dwFlags);
  ...
  WINAPI_SocketCode_int   WSASendTo(                       SOCKET s, LPWSABUF lpBuffers, DWORD dwBufferCount, LPDWORD lpNumberOfBytesSent, WINAPI_SendRecvFlags dwFlags, WINAPI_struct sockaddr* lpTo, int iToLen, LPWSAOVERLAPPED lpOverlapped, LPWSAOVERLAPPED_COMPLETION_ROUTINE lpCompletionRoutine);
  ...
  DWORD                   WSAWaitForMultipleEvents(        DWORD cEvents, WINAPI_WSAEVENT* lphEvents, BOOL fWaitAll, DWORD dwTimeout, BOOL fAlertable);
  BOOL                    WSAIsBlocking(                   );
