...
ffi.cdef [[
  ...
  PRThread*          PR_CreateThread(                     PRThreadType type, void _*__void*_ start, void* arg, PRThreadPriority priority, PRThreadScope scope, PRThreadState state, PRUint32 stackSize);
  ...
  const PRIOMethods* PR_GetDefaultIOMethods(              );
  ...
  PRSharedMemory*    PR_OpenSharedMemory(                 const char* name, PRSize size, PRIntn flags, PRIntn mode);
  ...
  PRStatus           PR_DeleteSharedMemory(               const char* name);
  PRFileMap*         PR_OpenAnonFileMap(                  const char* dirName, PRSize size, PRFileMapProtect prot);
  PRStatus           PR_ProcessAttrSetInheritableFileMap( PRProcessAttr* attr, PRFileMap* fm, const char* shmname);
  PRFileMap*         PR_GetInheritedFileMap(              const char* shmname);
  PRStatus           PR_ExportFileMapAsString(            PRFileMap* fm, PRSize bufsize, char* buf);
  PRFileMap*         PR_ImportFileMapFromString(          const char* fmstring);
  ...
  PRStack*           PR_CreateStack(                      const char* stack_name);
  ...
  PRStatus           PR_ProcessAttrSetCurrentDirectory(   PRProcess* attr, const char* dir);
  void               PR_ProcessAttrSetInheritableFD(      PRProcess* attr, PRFileDesc* fd, const char* name);
  PRProcess*         PR_CreateProcess(                    const char* path, char* const* argv, char* const* envp, const PRProcessAttr* attr);
